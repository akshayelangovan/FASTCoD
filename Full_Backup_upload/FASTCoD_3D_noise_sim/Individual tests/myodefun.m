...
% Adding Sensor Noise
x_n = x + randn(1)*0.01*10/3;
y_n = y + randn(1)*0.01*10/3;
z_n = z + randn(1)*0.01*10/3;
xdot_n = xdot + randn(1)*0.01*10/3;
ydot_n = ydot + randn(1)*0.01*10/3;
zdot_n = zdot + randn(1)*0.01*10/3;
theta_n = theta + randn(1)*0.01*(pi/3)/3;
phi_n = phi + randn(1)*0.01*(pi/3)/3;


xe = S.x_goal - x_n; % calculating x_error
% xedot  = (0 - xdot);
ye = S.y_goal - y_n; % calculating y_error
% yedot  = (0 - ydot);
ze = S.z_goal - z_n; % calculating z_error
% zedot = (0 - zdot);

[F,T_theta,T_phi] = getF(xe,xdot_n,ye,ydot_n,ze,zdot_n,theta_n,thetadot,phi_n,phidot,fis1,fis2,fis3,fis4,fis5,S);
% F = (M+m)*g;
% T_phi = 0; % for y error
