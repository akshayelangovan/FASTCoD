function fit = FitFun(R,P,fis1,fis2,fis3,fis4,fis5,fis6,fis7,initstate,costfuncname,S)
...
switch P.trainingcase % I can basically replace this with P.n_controllers
    ...
    case 'o' 
        for i = 1 : P.nvar/P.n_controllers
            fis7.rule(i).consequent(1) = R(i);
            fis7.rule(i).consequent(2) = R(i+1);
            fis7.rule(i).consequent(3) = R(i+2);
        end
end
...
for i = 1 : size(initstate,1)
    ...
    [t,z]=ode45(@P.ode,P.tspan,initstate(i,:)',options,fis1,fis2,fis3,fis4,fis5,fis6,fis7,S);
    if t(end)<P.T
        fit = -8e+08 + fit;
    else
