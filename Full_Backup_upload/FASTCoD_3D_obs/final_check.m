...
load('M9aa.mat');% Trained controller chromosome + fitness || M9a - motion tracking in XZ
% load('C3.mat');% Trained controller chromosome + fitness || C3 - stabilization in XZ
R0 = BestChrom.Gene;
...
load('M9ii.mat');% Trained controller chromosome + fitness || M9i - motion tracking in XZ
% load('C4i.mat');% Trained controller chromosome + fitness || C4i - stabilization in YZ
R1 = BestChrom.Gene;
...
% load('O1.mat')% Trained controller chromosom + fitness || For obstacle avoidance or waypoint generator
% R3 = BestChrom.Gene;
% for i = 1 : P.nrules
%     fis7.rule(i).consequent = R3(i);
% end
...
[t,z]=ode45(@P.ode,P.tspan,X0,options,fis1,fis2,fis3,fis4,fis5,fis6,fis7,S);
...
axis([-4 4 -4 4 -4 4])
view(30,30)
for i =1:length(S.world)
    plot3(S.world(i,1),S.world(i,2),S.world(i,3),'o')
    hold on
end
for i = 1:length(t)
    axis([-4 4 -4 4 -4 4])
